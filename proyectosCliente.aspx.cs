...
namespace despacho
{
    public partial class proyectosCliente : System.Web.UI.Page
    {
        ...
        {
            lvDetalles.DataSource = cProy.obtenerProyectosByIdCliente(id); 
            lvDetalles.DataBind();
        }
        ...
        protected void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            }catch(Exception ex)
            {
                if (ex.Message == "ok")
                {
                    ...
                    string errorMessage = "La acción se ha completado correctamente." + ex.Message;
                    ...
                    ScriptManager.RegisterStartupScript(this, GetType(), "refreshScript", "setTimeout(function() { location.reload(true); }, 2000);", true);

                    
                }
                else
                {
                    string errorMessage = "Verifica que la informacón ingresada sea correcta " + ex.Message;
                    ScriptManager.RegisterStartupScript(this, GetType(), "showErrorModal", $"$('#errorMessage').text('{errorMessage}'); $('#errorModal').modal('show');", true);
                }
