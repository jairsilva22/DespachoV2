...
namespace despacho
{
    public partial class reporteCobranza : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!Request.QueryString["fact"].Equals("0"))
                {
                    if (!Request.QueryString["fact"].Equals("1"))
                        lblTS.Text = "Con factura";
                    else
                        lblTS.Text = "Sin factura";
                }
                else
                    lblTS.Text = "Todas";
        ...
        public DataTable getSolicitudesByCliente(int idCliente, DateTime fechaIni, int facturada = 0)
        {
            try
            {
                DataTable dt = new DataTable();
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["cnx"].ConnectionString))
                {
                    ...
                    string whereComand = "WHERE (s.eliminada IS NULL) AND (s.idCliente = @idCliente) AND (s.fecha > @FechaIni) ";
                    if (facturada.Equals(1))
                        whereComand += "AND (s.reqFac = 'SI')";
                    if (facturada.Equals(2))
                        whereComand += "AND (s.reqFac = 'NO')";
        ...
        private void llenarLV()
        {
            ...
            foreach (DataRow drC in dtC.Rows)
            {
                ...
                //TODO
                dtS = getSolicitudesByCliente(int.Parse(drC["id"].ToString()), fechaIni, int.Parse(Request.QueryString["fact"].ToString()));
