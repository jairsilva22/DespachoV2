...
if Request.QueryString("mes") <> "0" AND Request.QueryString("mes") <> "" then'mes
mes = " AND Month(f.fechacfd)<="&request.QueryString("mes")
m = Cint(Request.QueryString("mes"))
elseif Request.QueryString("mes") = "0" then'mes
mes = ""
m = ""
end if'mes

If Request.QueryString("yearemiso") <> "" Then
  ano = " AND YEAR(f.fechaAlta) = "& Request.QueryString("yearemiso")
End If
Dim saldo
saldo=0

if request.QueryString("moneda") <> "" then
    mnda = " AND f.moneda = "&request.QueryString("moneda")
else
    mnda = "" 
    saldoUSD = 0
    saldoMXN = 0
...
vctes="AND f.idcliente="&request.QueryString("CTES")
IF request.QueryString("CTES")="TODOS" THEN
vctes=""
END IF
...
if request.Form("moneda") <> "" then
    ...
    RecordsetMnda_cmd.CommandText = "SELECT * FROM dbo.moneda WHERE idmd="&request.QueryString("moneda")&" ORDER BY cMoneda"
    RecordsetMnda_cmd.Prepared = true
       ' response.Write(RecordsetMnda_cmd.CommandText)
...
Function exportaraExcel() 
	...
	Response.AddHeader "content-disposition", "inline; filename=ReporteCXC"&date&".xls" 
End Function
%>
<!DOCTYPE html >
<html>
<!-- InstanceBegin template="/Templates/plantilla-peisa-admin.dwt.asp" codeOutsideHTMLIsLocked="false" -->
<head>
...
<body>
    ...
    <strong>
        <p align="left">
            Al <%=Day(date()) & " de " & MonthName(Month(date())) & " del " & Year(date())  %><br />
            <%=moneda %>
        </p>
    </strong>
...
call Separar(cadena, sNumeros, sCadena)
     ...
     totalEMXN = 0
       ...
       RecordsetFact_cmd.CommandText = "SELECT f.idcliente, f.fechacfd, f.idfactura, f.folio, f.idruta, f.estatus, f.total, c.idcliente, c.nombrecliente, c.rfcCliente, f.retencion, f.iva, d.descripcion as x, f.moneda FROM dbo.factura f LEFT JOIN detallesPagoFacturas p ON f.idfactura = p.idfactura INNER JOIN clientes c ON c.idcliente=f.idcliente INNER JOIN documento d ON tipo_comprobante = d.iddocumento INNER JOIN formPago fp ON f.forma_pago = fp.idpago AND fp.formaPago = 'PPD' WHERE p.idfactura IS NULL and f.idcliente = "&Recordset1.fields.item("idcliente").value&" AND f.estcobranza <> 'Pagada' AND f.timbre = 'NO' AND d.tipo = 'ingreso' AND f.estatus = 'facturada' ORDER BY c.nombrecliente, fechacfd ASC"
        ...
        elseif monedaMXN = RecordsetFact.Fields.item("moneda").Value then
              ...
              if impP > 0 then
                    ...
                    <tr>
                        ...
                        <td align="right"><%=FormatNumber(saldoUSD) %></td>
                    </tr>
                    <% 
