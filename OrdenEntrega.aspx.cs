...
namespace despacho
{
    public partial class OrdenEntrega : System.Web.UI.Page
    {
        ...
        private void llenarDetalleSolicitud(int idDS)
        {
            ...
            txtCodigoProducto_AutoCompleteExtender.ContextKey = hfIdTipoProducto.Value;
            ...
            if (cDS.getCantidadEntregada(idDS) > 0)
            ...
            txtPrecioFIVA.Text = dtDS.Rows[0]["precioFIVA"].ToString();
            txtIVA.Text = dtDS.Rows[0]["iva"].ToString();
        ...
        protected void txtCodigoProducto_TextChanged(object sender, EventArgs e)
        {
            ...
            txtPrecioF.Text = hfPrecioU.Value;
            txtPrecioFIVA.Text = (double.Parse(txtPrecioF.Text) + double.Parse(txtPrecioF.Text) * double.Parse(txtIVA.Text)).ToString();
        ...
        private bool modDS()
        {
            try
            {
                ...
                decimal precioF, subtotal, iva, total, precioFIVA;
                precioF = decimal.Parse(txtPrecioF.Text);
                precioFIVA = decimal.Parse(txtPrecioFIVA.Text);
                ...
                cDS.precioFIVA = precioFIVA.ToString("0.00");
                cDS.subtotal = subtotal.ToString("0.00");
                cDS.total = total.ToString("0.00");
                if (cDS.getCantidadEntregada(cDS.id) > 0)
                    cDS.actualizarCantSolicitada(int.Parse(hfIdDS.Value), int.Parse(Request.Cookies["ksroc"]["id"]));
                else
                {
                    cDS.actualizarProdAndCantSolicitada(int.Parse(hfIdDS.Value), int.Parse(Request.Cookies["ksroc"]["id"]));
                    if (Request.QueryString["idOD"] != null) {
                        int idOD;
                        if (int.TryParse(Request.QueryString["idOD"], out idOD)) {
                            // Aquí puedes utilizar idOD de manera segura
                            cOD.actualizarProducto(int.Parse(Request.QueryString["idO"]), idOD, cDS.idProducto, int.Parse(Request.Cookies["ksroc"]["id"]));
                        }
                    }
        ...
        protected void txtPrecioF_TextChanged(object sender, EventArgs e) {
            //txtPrecioFIVA.Text = (double.Parse(txtPrecioF.Text) + double.Parse(txtPrecioF.Text) * double.Parse(txtIVA.Text)).ToString();
        }
    }
}