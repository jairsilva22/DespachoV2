...
if request.QueryString("anio") <> "" then
    anio = " AND year(fechasellado) = " & request.QueryString("anio")
else
    anio = ""
end if
...
Recordset1_cmd.CommandText = "SELECT * FROM dbo.factura WHERE idfactura > 0 AND tipo_comprobante = "&documento & anio &" AND estatus='Facturada' order by folio asc"
...
Recordset2_cmd.CommandText = "SELECT * FROM dbo.documento WHERE iddocumento = "&documento 
'response.write(Recordset1_cmd.CommandText)
...
Recordset2_numRows = 0
    ...
    if not Recordset2.eof then
        descDoc = Recordset2.fields.item("descripcion").value
    else
        descDoc = ""
    end if
...
if not Recordset3.eof then
        empresa = Recordset3.fields.item("nombre").value
    else
        empresa = ""
    end if
%>
...
<head>
    ...
    <title>Reporte de Notas de cr&eacute;dito</title>
    <style type="text/css">
        .stilo1 {
            font-size: 12px;
            font-weight: bold;
...
<body>
    <table width="700" border="0">
            ...
            <td>
                <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                    <tr align="center" valign="bottom">
                        ...
                        <td colspan="2" class="titulos"><%=empresa %></td>
                    </tr>
                    <tr align="center">
                        <td height="25" colspan="2" valign="bottom">REPORTE DE NOTAS DE <%=UCase(descDoc) %>&nbsp;
                        </td>
        ...
        Set Recordset3_cmd = Server.CreateObject ("ADODB.Command")
        Recordset3_cmd.ActiveConnection = MM_Conecta1_STRING
        Recordset3_cmd.CommandText = "SELECT * FROM dbo.clientes WHERE idcliente="&recordset1.fields.item("idcliente").value 
...
'response.write(total)
    ...
    montoSub = montoSub + subTotal
    montoIva = montoSub * .16
    montototal = montoSub + montoIva
  response.Flush()
...
Wend
                    ...
                    <tr align="center" class="stilo2" bgcolor="#CCCCCC">
                        ...
                        <td align="right"><%=formatnumber(montoSub,2)%>&nbsp;</td>
                    </tr>
                    <tr align="center" class="stilo2" bgcolor="#CCCCCC">
                        <td colspan="4" align="right">IVA</td>
                        <td align="right"><%=formatnumber(montoIva,2)%>&nbsp;</td>
                    </tr>
                    <tr align="center" class="stilo2" bgcolor="#CCCCCC">
                        <td colspan="4" align="right">TOTAL</td>
                        <td align="right"><%=formatnumber(montototal,2)%>&nbsp;</td>
                    </tr>
                </table>
            </td>
        </tr>
