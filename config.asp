...
if InsTr(valor,".") <> 0 then'subtotal
...
function datef2(fecha)
mes=month(fecha)
if cint(mes)<9 then mes ="0"&mes
dia=day(fecha)
if cint(dia)<9 then dia ="0"&dia
datef2=year(fecha)&"/"&mes&"/"&dia
end function

function timef(hora)
f=time()
f=Hour(hora)&":"&Minute(hora)
timef=f
end function

dim iva'Variable para guardar el iva
dim empconfi'Variabpara guardar el nombre de la empresa
dim pathconfi'Variable para guardar el path del servidor
dim cadena'Variabelapra guardar la cadena de conexion para el correo

'------------Recordset para consultar el config--------------------------------'
if Session("site_empresa") <> "" AND Session("site_empresa") <> "yyyyyyyya" then
Set RSConfig_cmd = Server.CreateObject ("ADODB.Command")
RSConfig_cmd.ActiveConnection = MM_Conecta1_STRING
RSConfig_cmd.CommandText = "SELECT * FROM dbo.configmenor WHERE idempresa = " &  Session("site_empresa")

RSConfig_cmd.Prepared = true

Set RSConfig = RSConfig_cmd.Execute

'if para valdiar que hay datos
if NOT RSConfig.EOF then
iva = RSConfig.Fields.Item("iva").Value
pathconfi = RSConfig.Fields.Item("path").Value
cadena = RSConfig.Fields.Item("cadena").Value
end if
end if
'-------------------------------------------------------------------------------'
...
<%  
	Dim Headline_Font_Face_Type 
	Dim Headline_Font_Size 
	Dim Headline_Font_Color
	Dim Author_Font_Face_Type
	Dim Author_Font_Size 
	Dim Author_Font_Color
	Dim Summary_Font_Face_Type 
	Dim Summary_Font_Size 
	Dim Summary_Font_Color
	Dim Link_Color 
	Dim Active_Link_Color 
	Dim Visited_Link_Color
	Dim Hover_Link_Color 
	Dim Link_Color_Decoration
	Dim Active_Link_Decoration
	Dim Visited_Link_Decoration
	Dim Hover_Link_Decoration 
	...
	Dim bienvenidos
	Dim bannerbien
	Dim Table_background
	DIM pgrafica
	DIM upload

	'Conexion access
strCon = MM_Conecta1_STRING

	Set objCon = Server.CreateObject ("ADODB.Connection")
	objCon.Open strCon
	set objRec = objCon.execute("select * from dbo.styles")	
	Headline_Font_Face_Type = objRec("Headline_Font_Face_Type") 
	Headline_Font_Size = objRec("Headline_Font_Size") 
	Headline_Font_Color = objRec("Headline_Font_Color")
	Author_Font_Face_Type = objRec("Author_Font_Face_Type")
	Author_Font_Size = objRec("Author_Font_Size") 
	Author_Font_Color = objRec("Author_Font_Color")
	Summary_Font_Face_Type = objRec("Summary_Font_Face_Type") 
	Summary_Font_Size = objRec("Summary_Font_Size") 
	Summary_Font_Color = objRec("Summary_Font_Color")
	Link_Color = objRec("Link_Color") 
	Active_Link_Color = objRec("Active_Link_Color")
	Visited_Link_Color = objRec("Visited_Link_Color")
	Hover_Link_Color = objRec("Hover_Link_Color")
	Link_Color_Decoration = objRec("Link_Color_Decoration") 
	Active_Link_Decoration = objRec("Active_Link_Decoration")
	Visited_Link_Decoration = objRec("Visited_Link_Decoration") 
	Hover_Link_Decoration = objRec("Hover_Link_Decoration") 
	Background_Color = objRec("Background_Color") 
	Table_Align = objRec("Table_Align") 
	Table_Width = objRec("Table_Width") 
	
	Borde= objRec("borde")
	ColorTabla=objRec("Table_color")
	ColorBorde=objRec("Borde_color")
	TableConf= objRec("Table_Conf")
	
	objRec.Close
	Set objRec = nothing

set objRec = objCon.execute("select * from dbo.settings ")	
	headerPage = objRec("headerPage")
	footerPage = objRec("footerPage")
	titlePage = objRec("titlePage")
	menu= objRec("menu")
	url = objRec("url")
    pgrafica = objRec("mensaje")
	upload= objRec("menu")
	objRec.Close
	Set objRec = nothing

dim strCon 'Conexion de bdatos
dim informacion 'se muestra la informacion especialmente al cliente
...
function updatebit() {
  document.form1.proveere.value = document.form1.itemlist.value;
}
</script>
<script language="JavaScript">
