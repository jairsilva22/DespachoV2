...
namespace despacho
{
    public partial class generarOD : System.Web.UI.Page
    {
        ...
        protected void lvDetalles_ItemCommand(object sender, System.Web.UI.WebControls.ListViewCommandEventArgs e)
        {
            try
            {
                ...
                if (e.CommandName.Equals("programar"))
                {
                    ...
                    lblUDM2.Text = arr[5];
                    hfIdUDM.Value = arr[6];
                        if (arr[9].ToLower().Contains("oncreto"))
                            lblRevenimiento3.Text = arr[7].ToString();
                        else
                            lblRevenimiento3.Text = "0";
                    ...
                    if (insertarODNuevo(0, getHoraD()))
                    {
                        lblMensaje.Text += "Se programó con éxito la orden";
                        setOrdenToProgramada();
                    }
                    else
                    {
                        lblMensaje.Text += "Ocurrió un problema, favor de intentarlo nuevamente";
                    }
                }
            }
            catch (Exception)
        ...
        private bool insertarODNuevo(float qty, string sHora)
        {
            cargarDatosODManual();
            PanelDatosOD.Visible = false;
            cOD.idSucursal = int.Parse(hfIdSucursal.Value);
            cOD.fecha = DateTime.Parse(txtFechaDosificacion.Text);
            cOD.hora = lblHora.Text;
            cOD.idOrden = int.Parse(hfIdOrden.Value);
            cOD.idProducto = int.Parse(hfIdProducto.Value);
            cOD.cantidad = qty;
            cOD.idUDM = int.Parse(hfIdUDM.Value);
            ...
            cOD.idEstadoDosificacion = 1;
            cOD.idUnidadTransporte = 1;
            cOD.revenimiento = float.Parse(lblRevenimiento3.Text);
