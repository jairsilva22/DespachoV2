...
namespace despacho
{
    public partial class reporteEstadoResultados : System.Web.UI.Page
    {
        ...
        private void llenarLV()
        {
                ...
                for (int i = 1; i <= 4; i++)
                {
                    ...
                    for (int j = 1; j <= 12; j++)
                    {
                        ...
                        if (i.Equals(4))
                        {
                            ...
                            if (ingresoDescuentosM < 0) {
                                arrVN[j - 1] = ingresoDescuentosM;
                            }
                            else {
            ...
            if (continuar)
            {
                DataTable dtGastoVentas = new DataTable();
                DataTable dtNomina = new DataTable();
                ...
                for (int j = 1; j <= 12; j++)
                {
                    ...
                    //Obtenemos todas las salidas de compras hacia el estado de resultados de Ventas
                    dtGastoVentas = cRR.getSalidasCompras(firstDayOfMonth.ToString("yyyy-MM-dd"), lastDayOfMonth.ToString("yyyy-MM-dd"), 3, idSucursalCompras);
                    //Obtenemos Nomina
                    ...
                    cant = 0;
                    cantM = 0;
                    foreach (DataRow drC in dtGastoVentas.Rows)
                    {
                        cant = 0;
                        cant = float.Parse(drC["total"].ToString());
                        cantM = cantM + cant;
                    }
                    cantNominaM = 0;
                    foreach (DataRow drN in dtNomina.Rows)
            ...
            if (continuar)
            {
                DataTable dtGastoAdmon = new DataTable();
                DataTable dtNomina = new DataTable();
                ...
                for (int j = 1; j <= 12; j++)
                {
                    ...
                    //dtCompras = cRR.getBitacora(firstDayOfMonth.ToString("yyyy-MM-dd"), lastDayOfMonth.ToString("yyyy-MM-dd"), idSucursal);
                    //Obtenemos todas las salidas de compras hacia el estado de resultados de Ventas
                    dtGastoAdmon = cRR.getSalidasCompras(firstDayOfMonth.ToString("yyyy-MM-dd"), lastDayOfMonth.ToString("yyyy-MM-dd"), 2, idSucursalCompras);
                    //Obtenemos Nomina
                    ...
                    cant = 0;
                    cantM = 0;
                    foreach (DataRow drC in dtGastoAdmon.Rows)
                    {
                        cant = 0;
                        cant = float.Parse(drC["total"].ToString());
                        cantM = cantM + cant;
