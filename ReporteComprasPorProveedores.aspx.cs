...
namespace despacho
{
    public partial class ReporteComprasPorProveedores : System.Web.UI.Page
    {
        int idSucursal = 0;
        ...
        public void llenarLV()
        {
            ...
            if (perfil != "1" && perfil != "1007") {
                ...
                foreach (DataRow row in dtConexiones.Rows) {
                    ...
                    llenarDataTable(dtCTA, dtcuenta, idSucursal);
                }
            }
            //Totales
        ...
        public void llenarDataTable(DataTable dtCTA, DataTable dtcuenta, int sucursal)
        {
            ...
            if (dtCTA == null)
            {
                ...
                rw["FECHA"] = "#";
                rw["CFOLIO"] = "#";
                rw["PROVEEDOR"] = "No hay datos";
                rw["CONCEPTO"] = "No hay datos";
                rw["MONEDA"] = "#";
                rw["FECHAPAGO"] = "#";
                rw["UNIDAD"] = "#";
                rw["TOTAL"] = "$0";
            ...
            else
            {
                ...
                for (int i = 0; i < (dtCTA.Rows.Count); i++)
                {
                    ...
                    rw["FECHAPAGO"] = dtCTA.Rows[i]["CFECHAVENCIMIENTO"].ToString();
                    rw["UNIDAD"] = dtCTA.Rows[i]["CUNIDADESCAPTURADAS"].ToString();
                    rw["SUCURSAL"] = Nombresucursal;
        ...
        public void llenarAgGrid(DataTable dt) {
            var reporteComprasP = new List<reporteComprasProveedores>();

            for (int i = 0; i < dt.Rows.Count; i++) {
                reporteComprasP.Add(new reporteComprasProveedores() { sucursal = dt.Rows[i]["SUCURSAL"].ToString(), 
                    proveedor = dt.Rows[i]["PROVEEDOR"].ToString(), 
                    fecha = dt.Rows[i]["FECHA"].ToString(), 
                    factura = dt.Rows[i]["CFOLIO"].ToString(), 
                    concepto = dt.Rows[i]["CONCEPTO"].ToString(), 
                    total = dt.Rows[i]["TOTAL"].ToString(), 
                    moneda = dt.Rows[i]["MONEDA"].ToString(), 
                    unidad = dt.Rows[i]["UNIDAD"].ToString() 
                });
            }

            var serializer = new JavaScriptSerializer();
            serializer.MaxJsonLength = 2147483647;
            var serializedResult = serializer.Serialize(reporteComprasP);
