...
namespace despacho
{
    public partial class ReportePrecioCliente : System.Web.UI.Page
    {
        ...
        protected void llenarLV()
        {
            ...
            lvPrecio.DataSource = cCl.ObtenerPrecioCliente(int.Parse(Request.Cookies["ksroc"]["idSucursal"]), param);
            lvPrecio.DataBind();
        }
        ...
        protected string formaDePago(int idForma)
        {
            return cFP.obtenerNombreFormasPagoByID(idForma);
        }
        protected string montoDescuento(int idCliente)
        {
            if(cCl.MontoFactorClienteByID(idCliente) != "" && cCl.MontoFactorClienteByID(idCliente) != null)
            {
                return cCl.MontoFactorClienteByID(idCliente);
            }
            else
            {
                return "00.00";
        ...
        protected string cantidadEntregada(int idCliente)
        {
            return cCl.cantidadEntregadaByIDCliente(idCliente);
        }
        protected string cantidadCargada(int idCliente)
        {
            return cCl.cantidadCargadaByIDCliente(idCliente);
        }
        protected void exportarExcel()
        {
            ...
            else
            {
                try
                {
                    ...
                    Response.Clear();
                    Response.AddHeader("content-disposition", "attachment;filename=ReportePrecioCliente" + DateTime.Today.ToShortDateString() + ".xls");
