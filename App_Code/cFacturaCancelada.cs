...
namespace despacho
{
    public class cFacturaCancelada
    {
        //variables
        ...
        public DataTable reporteFacturasCanceladas(string param)
        {
            try
            {
                ...
                comando = "SELECT A.fechaSolicitud, A.fechaCancelado, A.folio, A.total, A.CodigoCan, D.descripcion, C.nombreCliente, C.rfcCliente, " +
                        "A.observaciones FROM facturaCancelada AS A INNER JOIN factura AS B ON B.idfactura = A.idfactura INNER JOIN clientesFacturacion AS C ON C.idCliente = " +
