...
namespace despacho
{
    public class cRemisionBlock
    {
        ...
        //string cmoneda;
        //Agregado por Enrique Sandoval
        cDosificacion cDOS = new cDosificacion();

        //Constructor
        public cRemisionBlock()
        {
            cadena = ConfigurationManager.ConnectionStrings["cnx"].ConnectionString;
        }
        public string generarPdf() {
            /*Corroborar si la ordenDosificacion viene en grupoViaje, para obtener todas las demás del mismo grupo
             * de esta manera, agregar los demás productos en el PDF y luego en la impresión
             * Agregado por Enrique Sandoval el 20-10-2023
             */
            bool isGrupoViaje = cDOS.isTravelGroup(idOD);
            DataTable productsViaje = null;
            if (isGrupoViaje) {
                int grupoViaje = cDOS.getTravelGroup(idOD);
                productsViaje = cDOS.obtenerODSinDuplicados(cOD.getProductoPDFGrupoViaje(idOrden, grupoViaje), "id");
            }
            ...
            Font fontDatos = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 8, BaseColor.BLACK));
            Font fontDatos1 = new Font(FontFactory.GetFont(FontFactory.TIMES_BOLD, 10, BaseColor.BLACK));
            Font fontDatos2 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 12, BaseColor.BLACK));
            Font fontDatos3 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 7, Font.BOLD, BaseColor.WHITE));
            Font fontDatos4 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 6, Font.BOLD, BaseColor.WHITE));
            Font fontDatos5 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 7, Font.BOLD, BaseColor.BLACK));
            Font fontDatos6 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 6, Font.BOLD, BaseColor.BLACK));
            Font fontDatosCot = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 12, Font.BOLD, BaseColor.BLACK));
            Font fontColObservaciones = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 7, Font.BOLD, BaseColor.BLACK));
            Font fontObservacines = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 10, BaseColor.BLACK));
            Font fontFolio = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 12, BaseColor.RED));
            Font fontObs = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 6, BaseColor.BLACK));
            Font fontTitulo = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 8, BaseColor.BLACK));
            Font fontTitulo1 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 8, BaseColor.WHITE));
            Font fontTitulo2 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 12, BaseColor.WHITE));
            Font fontTitulo0 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 9, BaseColor.WHITE));
            Font fontTitulo01 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 9, BaseColor.BLACK));
            Font stylo32 = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 26, Font.BOLD, BaseColor.RED));

            Font fontDatosE = new Font(FontFactory.GetFont(pathLetter + "arial.ttf", 10, Font.BOLD, BaseColor.WHITE));
            ...
            try {
                ...
                folio = dtRem.Rows[0]["folioR"].ToString();
                ...
                if (!Directory.Exists(path + "\\Remisiones\\" + idSucursal)) {
                    Directory.CreateDirectory(path + "\\Remisiones\\" + idSucursal);
                }
                //varificar si existe la carptea si no crearla
                if (!Directory.Exists(path + "\\Remisiones\\" + idSucursal + "\\Backup")) {
                    Directory.CreateDirectory(path + "\\Remisiones\\" + idSucursal + "\\Backup");
                }

                //verificamos si existe el archivo
                if (File.Exists(path + "\\Remisiones\\" + idSucursal + "\\" + nombrePDF)) {
                    oldFile = nombrePDF;
                    newFile = nombreDoc + folio.ToString() + "_" + DateTime.Now.ToString("yyyyMMddHHmmss") + extencion;
                    File.Copy(path + "\\Remisiones\\" + idSucursal + "\\" + oldFile, path + "\\Remisiones\\" + idSucursal + "\\Backup\\" + newFile, true);
                    //si existe lo borramos y creamos uno nuevo
                    File.Delete(nombrePDF);
                    pdf = PdfWriter.GetInstance(document, new FileStream(path + "\\Remisiones\\" + idSucursal + "\\" + nombrePDF, FileMode.Create, FileAccess.Write, FileShare.None));
                }
                else {
                ...
                imagenLogo.ScaleAbsolute(120, 50);
                ...
                PdfPCell cellCotizacion1 = new PdfPCell(new Phrase(dtRem.Rows[0]["folioR"].ToString(), fontFolio)); //Celda para guardar el texto Fecha
                PdfPCell cellFecha = new PdfPCell(new Phrase("FECHA:", fontDatos1)); //Celda para guardar el texto Remisión
                PdfPCell cellFecha1 = new PdfPCell(new Phrase(dtRem.Rows[0]["fecha"].ToString().Substring(0, 10), fontObservacines)); //Celda para guardar el texto Fecha
                //      '-----------------------------------------------------------------------
                ...
                cellFecha.HorizontalAlignment = Rectangle.ALIGN_CENTER;// 'Mostra bordes seleccionados
                cellFecha.Border = Rectangle.NO_BORDER;// 'Mostra bordes seleccionados
                cellFecha1.HorizontalAlignment = Rectangle.ALIGN_CENTER;// 'Mostra bordes seleccionados
                cellFecha1.Border = Rectangle.NO_BORDER;// 'Mostra bordes seleccionados
                ...
                tablaFolio.AddCell(cellFecha);// 'Agrega COLUMNA      
                tablaFolio.AddCell(cellFecha1);// 'Agrega COLUMNA  
                ...
                PdfPTable tablaRV = new PdfPTable(4);
                tablaRV.WidthPercentage = 100;// 'Ancho de la tabla
                //tablaRV.DefaultCell.FixedHeight = 80f;

                //celdas de la tabla de articulos-------------------------------------------------
                PdfPCell cellRV1 = new PdfPCell(new Phrase("RECIBE", fontTitulo1)); //Celda para guardar el texto Carga Inicial
                PdfPCell cellRV2 = new PdfPCell(new Phrase("", fontDatos)); //Celda para guardar el texto Formula
                PdfPCell cellRV3 = new PdfPCell(new Phrase("VENDEDOR", fontTitulo1)); //Celda para guardar el texto Carga Inicial
                PdfPCell cellRV4 = new PdfPCell(new Phrase(dtRem.Rows[0]["vendedor"].ToString(), fontDatos)); //Celda para guardar el texto Formula

                //------------------------------------------------------------------------------------
                //Propiedades de las celdas de la tabla de articulos------------------------------------
                cellRV1.BackgroundColor = new BaseColor(34, 46, 112); //Color del borde
                cellRV1.Border = 12; //Mostrar bordes seleccionados
                cellRV1.HorizontalAlignment = Rectangle.ALIGN_RIGHT; //Aliniar
                cellRV1.BorderColor = new BaseColor(0, 0, 0); //Color del borde
                //cellRV1.FixedHeight = 20f;
                //Propiedades de las celdas de la tabla de articulos------------------------------------
                cellRV2.Border = 12; //Mostrar bordes seleccionados
                cellRV2.HorizontalAlignment = Rectangle.ALIGN_CENTER; //Aliniar
                cellRV2.BorderColor = new BaseColor(0, 0, 0); //Color del borde
                //Propiedades de las celdas de la tabla de articulos------------------------------------
                cellRV3.BackgroundColor = new BaseColor(34, 46, 112); //Color del borde
                cellRV3.Border = 12; //Mostrar bordes seleccionados
                cellRV3.HorizontalAlignment = Rectangle.ALIGN_RIGHT; //Aliniar
                cellRV3.BorderColor = new BaseColor(0, 0, 0); //Color del borde
                //Propiedades de las celdas de la tabla de articulos------------------------------------
                cellRV4.Border = 12; //Mostrar bordes seleccionados
                cellRV4.HorizontalAlignment = Rectangle.ALIGN_CENTER; //Aliniar
                cellRV4.BorderColor = new BaseColor(0, 0, 0); //Color del borde

                colorB = color1;

                int[] anchoRV = { 25, 25, 25, 25 };// 'Tamaños de las celdas de la tabla Gral

                //Inserta los valores en la factura---------------------------------------------------
                tablaRV.AddCell(cellRV1);
                tablaRV.AddCell(cellRV2);
                tablaRV.AddCell(cellRV3);
                tablaRV.AddCell(cellRV4);

                //tablaRV.SpacingBefore = 5;
                document.Add(tablaRV);// Agrega la tabla al documento
                //'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                ...
                //celdas de la tabla de articulos-------------------------------------------------
                PdfPCell cellG21 = new PdfPCell(new Phrase("PRODUCTO", fontTitulo1)); //Celda para guardar el texto Carga Inicial
                PdfPCell cellG22 = new PdfPCell(new Phrase("CARGA", fontTitulo1)); //Celda para guardar el texto Formula
                PdfPCell cellG23 = new PdfPCell(new Phrase("CARGA ENTREGADA", fontTitulo1)); //Celda para guardar el texto Carga Inicial
                PdfPCell cellG24 = new PdfPCell(new Phrase("CARGA SOLICITADA", fontTitulo1)); //Celda para guardar el texto Formula
                ...
                int[] anchoGral2 = { 25, 10, 15, 15, 15, 20 };// 'Tamaños de las celdas de la tabla Gral
                ...
                if (isGrupoViaje && productsViaje != null) {
                    Dictionary<int, string> cantidades = cDOS.obtenerCantEntregadaVariasOD(idOrden);
                    foreach (DataRow dr in productsViaje.Rows) {
                        string cantidadE = cantidades[int.Parse(dr["idProducto"].ToString())];
                        //celdas de la tabla de Customers Columna1------------------------------------------------- modificar cantidad Entregada - Luis Moctezuma 09-03-2022
                        PdfPCell cellG2DATA1 = new PdfPCell(new Phrase(dr["codigo"].ToString(), fontDatos));
                        PdfPCell cellG2DATA2 = new PdfPCell(new Phrase(dr["cantidad"].ToString(), fontDatos));
                        PdfPCell cellG2DATA3 = new PdfPCell(new Phrase(cantidadE, fontDatos)); //Cantidad Entregada tomada de la suma de los viajes
                        //PdfPCell cellG2DATA3 = new PdfPCell(new Phrase(dr["cantidadEntregada"].ToString(), fontDatos)); //Cantidad Entregada tomada de la tabla
                        PdfPCell cellG2DATA4 = new PdfPCell(new Phrase(dr["cantOrdenada"].ToString(), fontDatos));
                        PdfPCell cellG2DATA5 = new PdfPCell(new Phrase(dr["nombreUnidadT"].ToString(), fontDatos));
                        PdfPCell cellG2DATA6 = new PdfPCell(new Phrase(dr["chofer"].ToString(), fontDatos));
                    ...
                    //celdas de la tabla de Customers Columna1------------------------------------------------- modificar cantidad Entregada - Luis Moctezuma 09-03-2022
                    PdfPCell cellG2DATA1 = new PdfPCell(new Phrase(dtRem.Rows[0]["codigo"].ToString(), fontDatos));
                    PdfPCell cellG2DATA2 = new PdfPCell(new Phrase(dtRem.Rows[0]["cantidad"].ToString(), fontDatos));
                    PdfPCell cellG2DATA3 = new PdfPCell(new Phrase(cDOS.obtenerCantE(idOrden), fontDatos)); //Cantidad Entregada tomada de la suma de los viajes
                                                                                                            //PdfPCell cellG2DATA3 = new PdfPCell(new Phrase(dtRem.Rows[0]["cantidadEntregada"].ToString(), fontDatos)); //Cantidad Entregada tomada de la tabla
                ...
                //tablaGral2.SpacingBefore = 5;
                document.Add(tablaGral2);// Agrega la tabla al documento

                //'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

                PdfPTable tablaGral3 = new PdfPTable(2);
                ...
                PdfPCell cellG35 = new PdfPCell(new Phrase("ENTREGA", fontTitulo1)); //Celda para guardar el texto Carga Inicial
                ...
                int[] anchoGral3 = { 20, 15 };// 'Tamaños de las celdas de la tabla Gral
                ...
                PdfPTable tablaEntrega = new PdfPTable(2);

                tablaEntrega.HorizontalAlignment = Rectangle.NO_BORDER;
                tablaEntrega.WidthPercentage = 100;

                PdfPCell cellPepi = new PdfPCell(new Phrase("PEPI:", fontDatos));
                PdfPCell cellPepi1 = new PdfPCell(new Phrase("", fontDatos));
                PdfPCell cellLab = new PdfPCell(new Phrase("L.A.B:", fontDatos));
                PdfPCell cellLab1 = new PdfPCell(new Phrase("", fontDatos));

                /*cellPepi.Border = Rectangle.NO_BORDER;
                cellPepi1.Border = Rectangle.NO_BORDER;
                cellLab.Border = Rectangle.NO_BORDER;
                cellLab1.Border = Rectangle.NO_BORDER;*/
                int[] anchoE = { 20, 35 };// 'Tamaños de las celdas de la tabla Gral
                tablaEntrega.SetWidths(anchoE);
                tablaEntrega.AddCell(cellPepi);
                tablaEntrega.AddCell(cellPepi1);
                tablaEntrega.AddCell(cellLab);
                tablaEntrega.AddCell(cellLab1);

                PdfPCell cellE = new PdfPCell(tablaEntrega);
                ...
                PdfPCell cellG3DATA5 = new PdfPCell(cellE);
                ...
                //celdas de la tabla de Customers Columna1-------------------------------------------------
                PdfPCell cellRecibido1 = new PdfPCell(new Phrase("", fontDatos5));
                PdfPCell cellRecibido2 = new PdfPCell(new Phrase("_______________________________", fontDatos6));
                PdfPCell cellRecibido3 = new PdfPCell(new Phrase("Nombre y Firma de Recibido", fontDatos6));
                ...
                //'----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                //se crea tabla para ingresar hora salid y hora regreso 
                PdfPTable tablaHrSR = new PdfPTable(4);
                tablaHrSR.WidthPercentage = 100;
                //tablaHrSR.DefaultCell.FixedHeight = 80f;

                //buscamos la hora de salida
                string iniC = "";
                foreach (DataRow dr in dtRem.Rows) {
                    if (dr[30].ToString().Contains("Se Actualizó campo Estado de dosificacion a = 5"))
                        iniC = dr[31].ToString().Remove(0, 11);
                }
                DateTime hraSalida = DateTime.Parse(iniC).AddMinutes(10);
                PdfPCell cellHoraSalida = new PdfPCell(new Phrase("HRA. SALIDA", fontTitulo1));
                PdfPCell cellHoraSalida1 = new PdfPCell(new Phrase(hraSalida.ToShortTimeString().ToUpper(), fontDatos));
                PdfPCell cellHoraRegreso = new PdfPCell(new Phrase("HRA. REGRESO", fontTitulo1));
                PdfPCell cellHoraRegreso1 = new PdfPCell(new Phrase("", fontDatos));

                //Propiedades de las celdas de la horas------------------------------------
                cellHoraSalida.BackgroundColor = new BaseColor(34, 46, 112); //Color del borde
                cellHoraSalida.Border = 12; //Mostrar bordes seleccionados
                cellHoraSalida.HorizontalAlignment = Rectangle.ALIGN_CENTER; //Aliniar
                cellHoraSalida.BorderColor = new BaseColor(0, 0, 0); //Color del borde
                //cellHoraSalida.FixedHeight = 20f;

                //cellHoraSalida1.FixedHeight = 20f;

                cellHoraRegreso.BackgroundColor = new BaseColor(34, 46, 112); //Color del borde
                cellHoraRegreso.Border = 12; //Mostrar bordes seleccionados
                cellHoraRegreso.HorizontalAlignment = Rectangle.ALIGN_CENTER; //Aliniar
                cellHoraRegreso.BorderColor = new BaseColor(0, 0, 0); //Color del borde
                //cellHoraRegreso.FixedHeight = 20f;

                //cellHoraRegreso1.FixedHeight = 20f;

                tablaHrSR.AddCell(cellHoraSalida);
                tablaHrSR.AddCell(cellHoraSalida1);
                tablaHrSR.AddCell(cellHoraRegreso);
                tablaHrSR.AddCell(cellHoraRegreso1);

                document.Add(tablaHrSR);
                ...
                //celdas de la tabla de Customers Columna1-------------------------------------------------
                PdfPCell cellObservacionesD = new PdfPCell(new Phrase(dtRem.Rows[0]["comentarios"].ToString(), fontDatos));
                ...
                tablaEncabezado.SpacingBefore = 90;
                document.Add(tablaEncabezado);// ' Agrega la tabla al documento
                document.Add(tablaGral);// Agrega la tabla al documento
                document.Add(tablaRV);// Agrega la tabla al documento
                document.Add(tablaGral2);// Agrega la tabla al documento
