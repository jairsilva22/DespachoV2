...
namespace despacho
{
    public class Factura
    {
        ...
        public void buscarFactura()
        {
            try
            {
                comando = "SELECT idCliente, idempresa, iva, subtotal, total, serie, ISNULL(idusuario, 1) AS idusuario, forma_pago, metodo_pago, tipo_comprobante, UUID, ";
                comando += "tasa, asn, vendedor, ordenCompra, VA, retencion, obsCliente, moneda, cambio, tretencion, metodoPago, NumCtaPago, ";
                comando += "condicionesDePago, numero_certificado, folio FROM factura WHERE idfactura = @id";
        ...
        public void insertarFactura()
        {
            try
            {
                comando = "INSERT INTO factura(idcliente, fecha, idempresa, iva, subtotal, total, estatus, idusuario, fechaAlta, forma_pago, metodo_pago, ";
                comando += "tipo_comprobante, fechacfd, tasa, ASN, vendedor, ordenCompra, VA, retencion, obsCliente, moneda, cambio, tretencion, ";
                comando += "metodoPago, NumCtaPago, condicionesDePago) ";
                comando += "VALUES(@idcliente, GETDATE(), @idempresa, @iva, @subtotal, @total, 'Pendiente', @idusuario, GETDATE(), @forma_pago, @metodo_pago, ";
        ...
        public string nombreCliente(object factura)
        {
            try
            {
                comando = "SELECT nombreCliente FROM clientesFacturacion AS clientes JOIN factura ON factura.idcliente = clientes.idcliente WHERE idfactura = @id";
                using (SqlConnection conn = new SqlConnection(cadena))
                {
                    conn.Open();
                    using (SqlCommand cmd = new SqlCommand(comando, conn))
        ...
        public DataTable facturasAnticipo()
        {
            try
            {
                ...
                comando += " clientes.nombreCliente, clientes.rfcCliente, cadena_original, uuid FROM factura INNER JOIN  clientesFacturacion AS clientes ON factura.idcliente = clientes.idCliente";
        ...
        public void datosFactura()
        {
            try
            {
                comando = "SELECT A.folio, B.nombreCliente, B.rfcCliente, A.UUID, A.fechasellado, A.total, A.estatus, ISNULL(A.CodigoCan, '') AS CodigoCan, A.idempresa FROM factura AS A JOIN ClientesFacturacion AS B " +
